...
DIVIDEND_PREDICTION_COLUMNS_ZH = {
    ...
    "method_a_growth_window_years": "方法A_年增率計算年數",
...
# Yahoo Finance's own ^TWII page (2026-09-15 CORRECTION, see
# market_index_data.py's module docstring) showed that was wrong: Volume
# for both indices comes straight from yfinance, matching Yahoo Finance's
# own site, and its precise unit isn't independently confirmed the way
# FMTQIK's 股/元 units are (via that endpoint's own "hints" field) -- so
# no unit is claimed here rather than guessing one.
# TradeValue's label says 億元 (not 元) because the tab_market_index block
# above scales the displayed value by /1e8 before rendering (2026-09-15
# follow-up request: "transform the unit of total value of transaction
# into 100 million dollars") -- the on-disk CSV itself is still in raw
# NT dollars.
MARKET_INDEX_COLUMNS_ZH = {
    "Date": "日期", "Open": "開盤", "High": "最高", "Low": "最低",
    "Close": "收盤", "Volume": "成交量", "TradeValue": "成交金額（億元）",
}
...
st.sidebar.caption(
    ...
    "連線 TWSE/yfinance/FinMind，部分連線會刻意放慢速度 -- 詳見 config.py 的 "
    "REQUEST_DELAY_SECONDS -- 如果看起來很慢，請不要重複點擊。"
)

st.sidebar.caption(
    "一次抓取全部資料（股價／股利／新聞會同時進行，比一個一個點快很多；"
    "流通股數／市值需要用到當天的股價，所以接著抓；三大法人買賣超"
    "（2026-09-13 起改用 FinMind，一檔股票一次連線就能拿到整段歷史，"
    "不用再一天一天抓，第一次抓也只需要幾秒到幾十秒）最後抓）："
)
...
with tab_market_index:
    ...
    if idx_data is None:
        ...
        if filtered.empty:
            ...
            # 2026-09-15 CORRECTION: Volume is NO LONGER scaled/overwritten
            # here -- it's shown exactly as yfinance/market_index_data.py
            # returns it, for both TAIEX and TPEx. An earlier version of
            # this block (same day) both (a) had market_index_data.py
            # overwrite TAIEX's Volume with TWSE FMTQIK's whole-market
            # share count, and (b) divided the result by 1000 to show "張"
            # here -- the user compared this page's TAIEX Volume against
            # Yahoo Finance's own ^TWII page directly and found they no
            # longer matched, and that the numbers from BEFORE this
            # feature touched Volume at all had matched Yahoo Finance.
            # Re-verified live (WebFetch): Yahoo Finance's own history for
            # ^TWII shows real, non-zero Volume in the low millions
            # (e.g. ~4-7 million/day) -- the same order of magnitude
            # yfinance returns, and NOT the same figure as FMTQIK's
            # whole-TWSE-market total (which is a different, much larger
            # quantity -- summed shares across every listed stock, not
            # TAIEX's own reported volume). See market_index_data.py's
            # module docstring for the full account. TradeValue is
            # unaffected by this correction -- it still comes from FMTQIK
            # (TAIEX only) and is still shown in 億元, since yfinance has
            # no trading-value field at all to conflict with.
    ...
    st.caption(
        "開高低收／成交量來源說明：TAIEX 直接取自 yfinance／Yahoo "
        "Finance 網站顯示的數字，已與 Yahoo Finance 官網核對一致。"
        "櫃買指數（TPEx）的開高低收改為直接取自櫃買中心官方報表"
        "「櫃買指數(月查詢)」，不再透過 yfinance／Yahoo Finance 抓取"
        "——因為 ^TWOII 這個代碼近期持續無法從 yfinance 取得資料。"
        "櫃買指數的成交量目前沒有可靠的免費資料來源（^TWOII 故障前"
        "由 yfinance 提供），因此保留空白，不會用櫃買中心整體市場的"
        "成交張數（一個規模完全不同、約大上百倍的統計數字）冒充。"
...
with tab_prices:
    if not my_watchlist:
        ...
        if prices is None:
            ...
            st.caption(
                "資料來源為 FinMind（2026-09-13 起，原本是證交所三大法人買賣超日報 T86 "
                "本身，改用 FinMind 是因為它一次連線就能拿到一檔股票的整段歷史，速度快"
                "很多；FinMind 本身的資料也是彙整自證交所/櫃買中心，如果對特定日期的數字"
                "有疑問，可以直接對照證交所 T86 原始報表核對）。外資＝外陸資（不含外資自營"
                "商）＋外資自營商；投信、自營商為官方分類加總。回溯的時間長度跟股價一樣"
                "（config.py 的 PRICE_HISTORY_MONTHS）。"
            )
...
with tab_dividends:
    if not my_watchlist:
        ...
        if divs is None and pred is None:
            ...
            st.caption(
                "方法A：先算出「整年年增率」（近2年配息總額 vs. 前2年配息總額，再換算成"
                "年化增率；用2年而非1年是為了不被單一年度的異常配息帶偏），再依「下一次"
